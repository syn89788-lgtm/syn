...
BOT_VERSION = "OK_C7_STORAGE_RECOVERY_V12"
...
DATA_FILE = os.path.join(APP_DIR, "OK_C7_STORAGE.json")
...
def normalize_data(data):
    ...
    data["next_post_id"] = max(int(data.get("next_post_id", 1) or 1), max_id + 1 if max_id else 1)
    return data



def storage_candidates():
    """أماكن محتملة لملف التخزين القديم. لا نغير التخزين الأساسي، فقط نحاول الإنقاذ."""
    candidates = [
        DATA_FILE,
        os.path.join(BASE_DIR, "OK_C7_STORAGE.json"),
        os.path.join(os.getcwd(), "OK_C7_STORAGE.json"),
        os.path.join(os.path.expanduser("~"), ".ok_c7_clean_py_bot", "OK_C7_STORAGE.json"),
        os.path.join(os.path.expanduser("~"), ".ok_c7_list_bot", "OK_C7_STORAGE.json"),
        os.path.join(os.path.expanduser("~"), ".ok_c7_js_list_bot", "OK_C7_STORAGE.json"),
    ]
...
def safe_load_storage_file(path):
    try:
        ...
        print(f"failed reading storage candidate {path}: {e}", flush=True)
        return None


def find_best_storage_data():
    """يرجع ملف التخزين الذي يحتوي أكبر عدد منشورات."""
    best_data = None
    best_path = None
    best_count = -1

    for path in storage_candidates():
        data = safe_load_storage_file(path)
        if data is None:
            continue
    ...
    if best_data is not None:
        print(f"best storage candidate: {best_path} posts={best_count}", flush=True)
...
def load_data() -> Dict[str, Any]:
    # لا نثق بملف فاضي إذا كان فيه ملف قديم فيه منشورات أكثر.
    primary_data = safe_load_storage_file(DATA_FILE)
    best_data, best_path, best_count = find_best_storage_data()

    if primary_data is None:
        if best_data is not None:
            # انسخ أفضل ملف إلى التخزين الأساسي بدون حذف المنشورات.
            with open(DATA_FILE, "w", encoding="utf-8") as f:
                json.dump(best_data, f, ensure_ascii=False, indent=2)
            print(f"recovered storage from {best_path} into {DATA_FILE}", flush=True)
            return normalize_data(best_data)

        data = default_data()
        save_data(data)
        return data

    primary_count = len(primary_data.get("posts", []))

    if best_data is not None and best_count > primary_count:
        # وجدنا تخزين قديم فيه منشورات أكثر من الملف الحالي، نرجعه.
        with open(DATA_FILE, "w", encoding="utf-8") as f:
            json.dump(best_data, f, ensure_ascii=False, indent=2)
        print(
            f"restored richer storage from {best_path}: {best_count} posts > {primary_count}",
            flush=True,
        )
        return normalize_data(best_data)

    return normalize_data(primary_data)

def existing_posts_count():
    try:
        if not os.path.exists(DATA_FILE):
            return 0
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            d = json.load(f)
        return len(d.get("posts", [])) if isinstance(d, dict) else 0
    except Exception:
        return 0
...
def save_data(data):
    ...
    old_count = existing_posts_count()
    new_count = len(data.get("posts", []))

    if old_count > 0 and new_count < old_count and not DELETE_OPERATION_ACTIVE:
        print(f"Blocked unsafe save old={old_count} new={new_count}", flush=True)
        return False
...
def recover_storage_command(chat_id):
    primary_data = safe_load_storage_file(DATA_FILE)
    primary_count = len(primary_data.get("posts", [])) if primary_data else 0

    best_data, best_path, best_count = find_best_storage_data()

    if best_data is None:
        send_message(chat_id, "ما لقيت أي ملف تخزين قديم. إذا عندك OK_C7_STORAGE.json ارفعه مع main.py.")
        return

    if best_count > primary_count:
        with open(DATA_FILE, "w", encoding="utf-8") as f:
            json.dump(best_data, f, ensure_ascii=False, indent=2)
        send_message(
            chat_id,
            "تم استرجاع أفضل ملف تخزين لقيته ✅\n"
            f"المصدر:\n{best_path}\n\n"
            f"عدد المنشورات المسترجعة: {best_count}"
        )
        return

    send_message(
        chat_id,
        "ما لقيت ملف فيه منشورات أكثر من الحالي.\n\n"
        f"الملف الحالي: {primary_count} منشور\n"
        f"أفضل ملف لقيته: {best_count} منشور\n"
        f"المسار:\n{best_path}"
    )
...
def handle_command(message):
    ...
    if cmd == "/recover":
        if not is_owner_or_assistant(user):
            send_message(chat_id, "هذا الإجراء للمالك أو مساعد المالك فقط.")
            return
        recover_storage_command(chat_id)
        return
...
def handle_callback(query):
    ...
    if data.startswith("admin_"):
        ...
        if data == "admin_home":
            ...
            d["next_post_id"] = next_available_post_id(d)
            save_data(d)
            edit_message_text(chat_id, message_id, f"تم إصلاح الترقيم بدون حذف.\nالرقم القادم: {d['next_post_id']}", admin_back())
        elif data == "admin_ban":
...
def handle_message(message):
    ...
    if state == "create_link":
        if not text.startswith(("http://", "https://")):
            ...
            return
        d = load_data()
        post_id = next_available_post_id(d)
