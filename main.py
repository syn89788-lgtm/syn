...
# دعم اللغة، قائمة بوتاتي، الاشتراكات بالنجوم، تعديل الأسعار
...
def get_subscription_info(user_id):
    ...
    for plan, bots in SUBSCRIPTION_BOTS.items():
        if lang == 'ar':
            ...
            plan_name = plan.replace('_',' ').capitalize()
        info += f"- {plan_name}: {bots} بوت(s)\n"
    return info

def list_user_bots(user_id):
    user_bots = user_bots_db.get(user_id, [])
    msg_text = f"عدد البوتات لديك: {len(user_bots)}\n"
    for b in user_bots:
