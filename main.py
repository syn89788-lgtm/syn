...
BOT_USERNAME = "DR7Drb220BOT"
RAZERESPECT_URL = "https://t.me/RAZERESPECTBOT"
BOT_VERSION = "OK_C7_DIRECT_SAVE_REUSE_IDS_V15"
...
# مهم: لا تغيّر اسم ملف التخزين ولا مساره مع أي تحديث أو تغيير توكن.
# طالما OK_C7_STORAGE.json موجود بجانب main.py، المنشورات والأرقام تبقى كما هي.
DATA_FILE = os.environ.get("OK_C7_STORAGE_FILE") or os.path.join(APP_DIR, STORAGE_FILE_NAME)
...
def start_health_server():
    # Render Web Service يحتاج يشوف بورت مفتوح، حتى لو البوت شغال polling.
...
def read_existing_data_raw():
    try:
        ...
        print("read_existing_data_raw failed:", e, flush=True)
        return None
...
def next_available_post_id(data):
...
def next_post_id_value(data):
    # الرقم القادم هو أصغر رقم فاضي.
    # مثال: إذا انحذف المنشور 2، المنشور الجديد يأخذ 2 بدل ما يقفز إلى 4.
    if not isinstance(data, dict):
        data = default_data()
    return next_available_post_id(data)


def allocate_post_id(data):
    post_id = next_available_post_id(data)
    data["next_post_id"] = next_available_post_id({**data, "posts": data.get("posts", []) + [{"id": post_id}]})
    return post_id
...
def save_data(data):
    ...
    if old_data is not None and not DELETE_OPERATION_ACTIVE:
        ...
        # أرقام المنشورات نفسها لا تتغير، لكن الرقم القادم مسموح يرجع لرقم محذوف.
        # هذا المطلوب: إذا انحذف 2، القادم يصير 2.
...
def build_post(message):
    # يبني المنشور من الرسالة نفسها أو من رسالة تم الرد عليها.
    # يدعم النص، الصور، الفيديو، الصوت، الفويس، والملفات.
    source = message.get("reply_to_message") or message
    caption = message.get("caption") or source.get("caption") or message.get("text") or source.get("text") or ""
    ...
    if caption:
        return {"content_type": "text", "text": caption, "top_text": caption}
    return None


def has_supported_content(message):
    source = message.get("reply_to_message") or message
    return bool(
        source.get("photo")
        or source.get("video")
...
def extract_url_from_entities(text, entities):
    text = text or ""
...
def extract_url_button(message):
    # إذا وصل زر رابط من تيليجرام نحفظه كما هو.
    # وإذا تيليجرام خبّى الزر في الرسائل المحولة، نحاول نلقط رابط من النص/الكابشن.
    # إذا ما وصل لا زر ولا رابط، نحفظ المنشور بدون زر بدل ما نعلّق المستخدم في سؤال وجواب.
    for src in (message, message.get("reply_to_message") or {}):
        markup = src.get("reply_markup") or {}
    ...
    url = extract_url_from_entities(text, entities)
    if url:
        return "اضغط للمشاهده", url

    # التقاط رابط عادي لو كان موجود داخل الكلام.
    for part in str(text).split():
        if part.startswith(("http://", "https://")):
            return "اضغط للمشاهده", part.strip()

    return "", ""


def save_new_post_from_message(message, require_button=False):
    ...
    if require_button and not (button_text and link):
        return None, "وصلت الرسالة بدون زر رابط ظاهر للبوت. تيليجرام أحيانًا يخفي أزرار الرسائل المحولة، هذا ليس سحرًا أسود من عندنا."
...
def should_direct_save_message(message, text):
    # حفظ مباشر للمالك ومساعده إذا أرسلوا منشور جاهز: صورة/فيديو/ملف/نص.
    # نستثني أزرار القائمة حتى لا تتحول كلمة مثل "منشوراتي" إلى منشور، لأن هذا سيكون إنجازًا محرجًا.
    menu_texts = {
        "صنع منشور 📌", "تعديل منشور 📝", "منشوراتي 📁", "معاينة منشور 👁",
        "إرسال منشور 📤", "إدارة القنوات 📡", "إلغاء / رجوع ↩",
        "الرجوع للقائمة الرئيسية ↩", "القائمة الرئيسية ↩",
    }
    if text in menu_texts:
        return False
    if text.startswith("/"):
        return False
    return has_supported_content(message)
...
def handle_command(message):
    ...
    if cmd == "/p":
        ...
        if not post:
            ...
            return
        send_message(chat_id, f"معاينة المنشور رقم {args[0]}:")
...
def handle_message(message):
    ...
    # حفظ مباشر: إذا المالك أو مساعده أرسل منشور جاهز، ينحفظ فورًا.
    # لو فيه زر رابط نحفظه، ولو ما وصل الزر من تيليجرام نحفظ المحتوى بدون تعطيل.
    if state is None and is_owner_or_assistant(user) and should_direct_save_message(message, text):
        post_id, error = save_new_post_from_message(message)
        if error:
            send_message(chat_id, error, reply_keyboard(user))
            return
        button_text, link = extract_url_button(message)
        extra = "\nتم حفظ الزر والرابط." if button_text and link else "\nتنبيه: ما وصل للبوت زر رابط، فانحفظ المحتوى بدون زر."
        send_message(chat_id, f"تم حفظ المنشور تلقائيًا ✅\nرقم المنشور: {post_id}{extra}", reply_keyboard(user))
        return
    ...
    if state == "create_content":
        if not is_owner_or_assistant(user):
            send_message(chat_id, "هذا الإجراء للمالك أو مساعد المالك فقط.")
            return
        if message_has_url_button(message):
            post_id, error = save_new_post_from_message(message)
            USER_STATE.pop(uid, None)
            if error:
                send_message(chat_id, error, reply_keyboard(user))
                return
            send_message(chat_id, f"تم حفظ المنشور المحول تلقائيًا ✅\nرقم المنشور: {post_id}", reply_keyboard(user))
            return
        post = build_post(message)
        if not post:
