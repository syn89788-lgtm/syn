...
BOT_VERSION = "OK_C7_RENDER_PORT_FIX_V14"
...
def next_post_id_value(data):
    ...
    return max(current_next, max(ids) + 1 if ids else 1)
...
def save_data(data):
    ...
    if old_data is not None and not DELETE_OPERATION_ACTIVE:
        ...
        # يمنع الرجوع للخلف في الرقم القادم، لأن البشر اكتشفوا طريقة لإفساد حتى الأرقام.
        old_next = int(old_data.get("next_post_id", 1) or 1)
        if int(data.get("next_post_id", 1) or 1) < old_next:
            data["next_post_id"] = old_next
...
def extract_url_button(message):
    # إذا حوّلت/نسخت رسالة فيها زر رابط، نحفظ نص الزر والرابط مباشرة.
    # بعض رسائل تيليجرام المحولة قد لا ترسل reply_markup للبوت، لكن إذا وصل الزر هنا يتم حفظه.
...
def handle_message(message):
    ...
    # حفظ مباشر: إذا المالك أو مساعده حول/نسخ رسالة فيها زر رابط، تنضاف كمنشور فورًا.
    # بدون ما نطلب نص زر ثم رابط، لأن هذا النوع من التعذيب الإداري لا فائدة منه.
    if state is None and is_owner_or_assistant(user) and message_has_url_button(message):
