...
def ask_to_upload_file(call):
    ...
    if bot_locked:
        bot.send_message(call.message.chat.id, "⚠️ البوت مقفل حالياً. الرجاء التواصل مع المطور @OK_C7.")
        return
...
def subscription_menu(call):
    ...
    bot.send_message(call.message.chat.id, build_subscription_info(), reply_markup=markup)
